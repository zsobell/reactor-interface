...
def register(port: int) -> Path | None:
    ...
    try:
        ...
        path = INSTANCES_DIR / f"{os.getpid()}.json"
...
def live_others(me: int | None = None) -> list[dict]:
    ...
    out: list[dict] = []
    try:
        entries = sorted(INSTANCES_DIR.glob("*.json"))
    except Exception:
        return out
...
def kill_others(me: int | None = None) -> list[int]:
