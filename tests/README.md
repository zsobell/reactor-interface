...
| `test_keithley_supplies.py` | The four DC supplies: *IDN? parsing, and above all WHEN their outputs are switched — on at pre-start, off at run end/abort, untouched by a reignite, and the sample bias only for a non-zero field. The "not cycled by a reignite" assertion is the load-bearing one |
