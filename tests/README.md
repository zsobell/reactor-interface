...
| `test_analysis_layout_api.py` | `/api/analysis_layout` — the Analysis page's plot grid being server-owned (2026-08-26) so every machine sees the same plots: round trip, last-write-wins between browsers, and a damaged file degrading to "nothing saved" rather than a 500. Drives the real FastAPI app over raw ASGI **without** running its lifespan, so it never touches the DAQ or the serial ports |
| `test_gas_simultaneous.py` | The Simultaneous gas order (2026-08-26): two gases sharing the whole window instead of dividing it — each at its own flow, nothing switching mid-exposure, on once for the run in EE-CVD, and a lone Simultaneous gas refusing the run before it starts |
| `test_shutdown_teardown.py` | `Supervisor.stop()` finishing even when a device will not disconnect (2026-08-27): every step bounded and named, the DAQ still released, and the whole teardown well inside the hard deadline in `__main__`. Written after the Shut down button left a zombie holding the DAQ and COM8-COM12 |
| `test_soft_open.py` | The Ar pneumatic being pulsed open rather than flipped open (2026-08-26): the pulse train's shape and timing, that it comes out of *every* open — manual, pre-start, recipe — that the pulses' closes do not zero a live Ar setpoint the way a real close does, and that zero pulses turns it back off |
...
| `test_run_estimate.py` | `/api/run/estimate` — the run length shown on an idle Run tab (2026-08-28): that it is the *same* builder's `cycle_seconds` the countdown will start from rather than a second copy of the arithmetic, that EE-CVD comes out shorter, that teardown stays out of it, and that a half-typed gas schedule answers "no number" instead of a 500. Raw ASGI, no lifespan |
...
- `test_merge_integration.py`: live-edit report stalls cannot delay hardware cleanup;
  accepted reports drain in order; gas names, frozen telemetry and settings belong
  to their own reactor instance.
