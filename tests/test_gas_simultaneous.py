...
async def main() -> int:
    ...
    async with VirtualReactor() as vr:
        ...
        async def observe_tick(seconds):
            if runner.progress.step_op == "electron_beam":
                exposure_flows.append((h2.commanded_sccm, n2.commanded_sccm))
            await original_tick(seconds)

        runner._tick = observe_tick
        try:
            await run_to_completion(vr, lambda: vr.sup.start_ald_run(dict(BOTH)))
        finally:
            runner._tick = original_tick
        ...
        # Final cleanup zeroes gases just before grounding the beam, after the
        # exposure budget is exhausted. Separate device timestamps can resolve
        # those commands or coincide on Windows, so strict timestamp inclusion
        # falsely classified cleanup as a mid-exposure switch. Observe actual
        # exposure samples and retain the exact transition-count checks above.
        c.check("both gases flow throughout every exposure sample",
                bool(exposure_flows) and all(flows == (H2_FLOW, N2_FLOW)
                                            for flows in exposure_flows), str(exposure_flows))
