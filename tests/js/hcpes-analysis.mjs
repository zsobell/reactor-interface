...
import {
  ...
  label, metricKeys, metricValue, pointConditionLines, rawCurrent, sliceGroups,
...
const source = {
  ...
  points:[
    {source_session:"neg",source_point:2,signed_order:1,signed_stage_bias_v:-10,
      "setpoint:mfc:ar":1,"setpoint:supply:grid_bias":100,"setpoint:supply:collimating":.0015,
    ...
    {source_session:"pos",source_point:2,signed_order:4,signed_stage_bias_v:10,
      "setpoint:mfc:ar":2,"setpoint:supply:grid_bias":100,"setpoint:supply:collimating":.0015,
...
assert.equal(label("setpoint:supply:collimating"), "collimating (mA)");
...
assert.ok(pointConditionLines(source,source.points[0]).includes("collimating (mA): 1.5"));
