...
assert.equal(axisDisplayScale({quantity:"current",unit:"A"}), 1);
assert.equal(axisDisplayUnit({quantity:"current",unit:"A"}), "A");
...
const plan = {
  ...
  initial_setpoints:{"mfc:ar":5,"supply:stage_bias":20,"supply:collimating":2},
  settings:{establishment:{trend_window_s:5,stable_window_s:20,maximum_wait_s:60,max_drift_a_per_min:.0001},
    parameter_change:{trend_window_s:3,stable_window_s:3,maximum_wait_s:10,max_drift_a_per_min:.0003},
...
const preview = {plan, estimate:{points:2, parameter_changes:4, qualified_samples:10,
  ...
  {target:"mfc:ar", label:"Ar", unit:"sccm", mode:"linear", count:2},
  {target:"supply:stage_bias", label:"Stage", unit:"V", mode:"fixed", count:1},
  {target:"supply:collimating", label:"Collimating", unit:"A", mode:"fixed", count:1},
  {target:"mfc:bg", label:"Background", unit:"sccm", mode:"locked_zero", count:1},
]};
assert.match(launchReview(preview, "positive-001"),
  /Stage wiring: \+ orientation[\s\S]*no added collection timer[\s\S]*INITIAL PLASMA CONDITION[\s\S]*Ar: 5 sccm[\s\S]*No qualified sweep samples[\s\S]*PLASMA ESTABLISHMENT[\s\S]*settling does not consume it[\s\S]*four HCPES support supplies output off/);
...
assert.match(root.innerHTML, /Initial plasma condition[\s\S]*used only to ignite/);
...
assert.match(root.innerHTML, /RoC rolling window/);
...
assert.match(root.innerHTML, /Collimating[\s\S]*1\.5[\s\S]*A/,
  "steering and collimating sweep blocks display amperes");
...
const initialAr={dataset:{initialTarget:"mfc:ar",axisScale:"1"},value:"7",closest:()=>null};
await root.events.get("input")({target:initialAr});
assert.equal(editor.selected().initial_setpoints["mfc:ar"],7,
  "initial plasma inputs update the saved startup condition");
const currentRow={dataset:{axis:"2"}};
const currentAxis={dataset:{axisValue:"value",axisScale:"1"},value:"2.5",
  closest:()=>currentRow,hasAttribute:()=>false};
await root.events.get("input")({target:currentAxis});
assert.equal(editor.selected().axes[2].value,2.5,
  "ampere sweep input is retained in stored amperes");
...
editor.updateStatus({running:false, phase:"complete",
  applied_setpoints:{"mfc:ar":0,"mfc:bg":0}});
assert.match(elements.get("hcpesLiveSetpoints").innerHTML, /Ar[\s\S]*0 sccm/);
assert.doesNotMatch(elements.get("hcpesLiveSetpoints").innerHTML, /Stage/,
  "cleanup clears de-energized supply commands while retaining zeroed MFCs");
editor.updateStatus({running:true, phase:"timed settle", session_id:"positive-001",
  observed_drift_a_per_min:null, drift_threshold_a_per_min:.001});
assert.equal(elements.get("hcpesLiveRate").textContent, "measuring…",
  "a missing trend is not displayed as an exact zero");
assert.doesNotMatch(elements.get("hcpesLiveRateCard").className, /good|bad/,
  "a missing trend cannot claim stable or unstable status");
