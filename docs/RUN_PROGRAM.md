...
However the sequence ends — success, an operator abort, or a crash — the beam
...
- **`reactor/control/recipe.py`** — the recipe engine.
  ...
  - `RecipeRunner` — runs setup → N×steps → teardown on its own asyncio
    task. One timed wait per step rather than a poll loop, so UI load can't
    stretch a dose; see "Timing: how a run stays on schedule" above.
    Pausable, abortable.
...
| Pump A after dose (s) | `pump_a_s` | 10 | EE-ALD: wall clock. EE-CVD: lit-time gated |
